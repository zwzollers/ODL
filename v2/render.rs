...
pub fn render_object_view (app: &mut TemplateApp, ui: &mut egui::Ui) {
    egui::Frame::canvas(ui.style()).show(ui, move |ui| {
        let (rect, resp) = ui.allocate_exact_size(egui::Vec2::splat(300.0), egui::Sense::all());
        ...
        if resp.hovered() {
            ...
            //     i.events.iter().find_map(|e| match e{ 
            //         egui::Event::MouseMoved(pos) => Some(*pos),
            //         _ => None,
            //     })
            // });
            // if let Some(d) = scroll {
            ...
            // app.camera_controller.handle_mouse_click( PointerButton::Primary, resp(egui::PointerButton::Primary));
            // app.camera_controller.handle_mouse_click( PointerButton::Middle, resp.clicked_by(egui::PointerButton::Middle));
        }
