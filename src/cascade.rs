...
use glam::dvec3;
use opencascade::{
    angle::Angle::Radians, mesh::Mesh, primitives::{Compound, IntoShape, Shape}, section, workplane::Workplane
};

pub fn test_cascade() -> Mesh {
    let model = {
    ...
    model.mesh().unwrap()
    //model.write_stl("test_stl/test.stl").unwrap();
}